...
import { STR, todayISO, buildFileName, formatDateAr } from '../i18n/ar.js?v=v2026-07-22.10';
import { el, progressBar, toast } from './components.js?v=v2026-07-22.10';
import { VARIANTS } from '../contracts.js?v=v2026-07-22.10';
import { getGenLibs } from '../vendor-loader.js?v=v2026-07-22.10';
import { resetRunData } from '../state.js?v=v2026-07-22.10';
...
import { buildLateLabsSection, triggerDownload } from './late-labs-section.js?v=v2026-07-22.10';
...
async function buildVariantSpec(model, variant) {
  const mod = await tryImport('../slidespec/build-spec.js?v=v2026-07-22.10');
...
async function makePptx(spec, variant, libs) {
  if (!spec) return null;
  const mod = await tryImport('../render/pptx-renderer.js?v=v2026-07-22.10');
...
async function makePdf(spec, variant, libs, host, onProgress, thumbs) {
  if (!spec) return null;
  const rMod = await tryImport('../render/html-renderer.js?v=v2026-07-22.10');
  const renderSlides = pickFn(rMod, ['renderSlides', 'renderSpec', 'renderHtml', 'render']);
  const pMod = await tryImport('../render/pdf-export.js?v=v2026-07-22.10');
