...
import { STR, todayISO, formatDateAr } from '../i18n/ar.js?v=v2026-08-31.4';
import { el, dropZone, fileSummaryCard, toast } from './components.js?v=v2026-08-31.4';
import { normTest } from '../contracts.js?v=v2026-08-31.4';
import { getPapa, getXLSX } from '../vendor-loader.js?v=v2026-08-31.4';
import { TAT_LOINC } from '../seeds/tat-lookup.js?v=v2026-08-31.4';
import { buildLateLabsSection } from './late-labs-section.js?v=v2026-08-31.4';
import { buildAutomationPanel } from './automation-panel.js?v=v2026-08-31.4';
...
const AUTOMATION_PIPELINE_URL = '../automation/pipeline.js?v=v2026-08-31.4';
...
async function ingestCsv(file) {
  ...
  const mod = await tryImport('../ingest/csv.js?v=v2026-08-31.4');
...
async function ingestTracker(file) {
  ...
  const mod = await tryImport('../ingest/xlsx.js?v=v2026-08-31.4');
...
function csvStats(orders) {
  ...
  for (const o of orders) {
    ...
    // EXACT literal, matching engine.js's cancelled test (was a /cancel/i regex,
    // which also swallowed e.g. 'Cancellation Requested' and let the upload
    // preview disagree with the deck). Statuses are matched exactly, never fuzzily.
    if (o.rawStatus === 'Order Cancelled') cancelled++;
  }
...
export async function render(container, ctx) {
  ...
  async function fetchLive() {
    ...
    try {
      const mod = await import('../ingest/grafana.js?v=v2026-08-31.4');
  ...
  async function enhanceUnmatched(unmatched, rowRefs, subtitleEl, actionsBar) {
    ...
    let mod;
    try {
      mod = await import('../ingest/tat-suggest.js?v=v2026-08-31.4');
  ...
  async function paintHero() {
    ...
    let out;
    try {
      const mod = await import('../engine/engine.js?v=v2026-08-31.4');
  ...
  async function runEngineAndGo() {
    ...
    try {
      ...
      if (!out) {
        try {
          const mod = await tryImport('../engine/engine.js?v=v2026-08-31.4');
          const compute = pickFn(mod, ['compute', 'runEngine', 'run']);
          if (compute) {
