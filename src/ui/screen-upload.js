...
import { STR, todayISO, formatDateAr } from '../i18n/ar.js?v=v2026-09-08.1';
import { el, dropZone, fileSummaryCard, toast } from './components.js?v=v2026-09-08.1';
import { normTest } from '../contracts.js?v=v2026-09-08.1';
import { getPapa, getXLSX } from '../vendor-loader.js?v=v2026-09-08.1';
import { TAT_LOINC } from '../seeds/tat-lookup.js?v=v2026-09-08.1';
import { buildLateLabsSection } from './late-labs-section.js?v=v2026-09-08.1';
import { buildAutomationPanel } from './automation-panel.js?v=v2026-09-08.1';
...
const AUTOMATION_PIPELINE_URL = '../automation/pipeline.js?v=v2026-09-08.1';
...
async function ingestCsv(file) {
  ...
  const mod = await tryImport('../ingest/csv.js?v=v2026-09-08.1');
...
async function ingestTracker(file) {
  ...
  const mod = await tryImport('../ingest/xlsx.js?v=v2026-09-08.1');
...
export async function render(container, ctx) {
  ...
  async function fetchLive() {
    ...
    try {
      const mod = await import('../ingest/grafana.js?v=v2026-09-08.1');
  ...
  async function enhanceUnmatched(unmatched, rowRefs, subtitleEl, actionsBar) {
    ...
    let mod;
    try {
      mod = await import('../ingest/tat-suggest.js?v=v2026-09-08.1');
  ...
  async function paintHero() {
    ...
    let out;
    try {
      const mod = await import('../engine/engine.js?v=v2026-09-08.1');
  ...
  async function runEngineAndGo() {
    ...
    try {
      ...
      if (!out) {
        try {
          const mod = await tryImport('../engine/engine.js?v=v2026-09-08.1');
          const compute = pickFn(mod, ['compute', 'runEngine', 'run']);
          if (compute) {
