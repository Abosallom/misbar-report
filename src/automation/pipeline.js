...
import { STR, todayISO, buildFileName } from '../i18n/ar.js?v=v2026-08-31.5';
import { VARIANTS, normTest } from '../contracts.js?v=v2026-08-31.5';
import { getGenLibs } from '../vendor-loader.js?v=v2026-08-31.5';
...
async function buildVariantSpec(model, variant) {
  const mod = await tryImport('../slidespec/build-spec.js?v=v2026-08-31.5');
...
async function makePptx(spec, variant, libs) {
  if (!spec) return null;
  const mod = await tryImport('../render/pptx-renderer.js?v=v2026-08-31.5');
...
async function makePdf(spec, variant, libs, host, onProgress, onSlides) {
  if (!spec) return null;
  const rMod = await tryImport('../render/html-renderer.js?v=v2026-08-31.5');
  const renderSlides = pickFn(rMod, ['renderSlides', 'renderSpec', 'renderHtml', 'render']);
  const pMod = await tryImport('../render/pdf-export.js?v=v2026-08-31.5');
...
const DEFAULT_DEPS = Object.freeze({
  loadGrafana: () => import('../ingest/grafana.js?v=v2026-08-31.5'),
  loadEngine: () => tryImport('../engine/engine.js?v=v2026-08-31.5'),
  loadReportModel: () => import('../model/report-model.js?v=v2026-08-31.5'),
  loadDeltaBaseline: () => tryImport('../model/delta-baseline.js?v=v2026-08-31.5'),
  ...
  loadDeltaWindow: () => tryImport('../model/delta-window.js?v=v2026-08-31.5'),
  loadTaskLifecycle: () => tryImport('../model/task-lifecycle.js?v=v2026-08-31.5'),
  loadLateLabs: () => import('../export/late-labs.js?v=v2026-08-31.5'),
  loadTatSuggest: () => tryImport('../ingest/tat-suggest.js?v=v2026-08-31.5'),
  loadTatLoinc: () => tryImport('../seeds/tat-lookup.js?v=v2026-08-31.5'),
  // Track 5's module; absent until it ships → the emails step reports 'skip'.
  loadEmlDraft: () => tryImport('../export/eml-draft.js?v=v2026-08-31.5'),
  ...
  loadSendoutMaster: () => tryImport('../ingest/sendout-master.js?v=v2026-08-31.5'),
  ...
  loadLabContacts: () => tryImport('../seeds/lab-contacts.js?v=v2026-08-31.5'),
  loadDownload: () => tryImport('../ui/late-labs-section.js?v=v2026-08-31.5'),
  produceReportFiles,
  now: () => Date.now(),
});
...
export async function runAutomation({
  ...
  async function stepEngine() {
    ...
    // Decrypt the send-out catalogue with the same data key the snapshot uses.
    // Never fatal: null just drops the two slides from this run's deck.
    let sendoutMaster = null;
    try {
      const soMod = typeof D.loadSendoutMaster === 'function' ? await D.loadSendoutMaster() : null;
      const load = pickFn(soMod, ['loadSendoutMaster', 'fetchSendoutMaster']);
      if (typeof load === 'function') {
        sendoutMaster = await load(((settings.grafana || {}).dataKey) || '');
      }
    } catch (e) {
      errors.push(`sendout: ${(e && e.message) || String(e)}`);
    }
